...
public class Main {
    ...
    static void menu(User currentUser, MyDatabase myDatabase) throws SQLException {
        ...
        while (!isLoggedIn) {
            ...
            if (firstMenuSelection.equals("1")) {
                ...
                myDatabase.createUser(userList.get(userList.size() - 1)); //After Registration, adding new user in database
        ...
        while (true){
            ...
            System.out.println("2-Show Your Tasks");
            System.out.println("3-Create New Task");
            System.out.println("4-Update Current Task");
            System.out.println("5-Search Task");
            System.out.println("6-Assign Task");
            System.out.println("7-Add Editor for Task");
            System.out.println("8-Generate Reports");
            System.out.println("0-Log out");
            Reminder.startReminderService();
            ...
            switch (menuSelection) {
                ...
                case "1":
                    ...
                    break;
                case "2":
                    // shows task only user is in.
                    showTask(currentUser);
                    break;
                case "3":
                ...
                case "7":
                    ...
                    break;
                case "8":
                    System.out.println("Generating Reports..."); // Generate Reports
                    Report.generateAllReports(taskList);
                    break;
                default:
                    System.out.println("Wrong Input!!!");
                    break;
    ...
    static void createTask(User currentUser, MyDatabase myDatabase) throws SQLException {
        ...
        System.out.println("Enter Task Phase\n1-DONE\n2-DOING\n3-TODO");
    ...
    static void updateTask(User currentUser, int taskId, MyDatabase myDatabase) throws SQLException {
        for (Task task : taskList){ // It iterates all Task list and finds by Task Id.
            if(task.getTaskId()==taskId){
                ...
                Scanner input = new Scanner(System.in);
                System.out.println("Enter new Task name (" + task.getTaskName() + ")");
                String taskName = input.nextLine();
                System.out.println(task.getTaskName()+" changed to "+taskName);
                task.setTaskName(taskName);

                System.out.println("Enter new Task description (" + task.getDescription() + ")");
                String taskDescription = input.nextLine();
                System.out.println(task.getDescription()+" changed to "+taskDescription);
                task.setDescription(taskDescription);

                System.out.println("Enter new Task Priority("  + task.getPriority() + ")"+  "\n1-HIGH\n2-MODERATE\n3-LOW");
                String priority = input.nextLine();
                String oldPriority=task.getPriority().toString();
                task.setPriority(priority);
                System.out.println(oldPriority+" changed to "+task.getPriority());

                System.out.println("Enter new Task Start Date(yyyy-mm-dd) (" + task.getStart() + ")");
                String start = input.nextLine();
                System.out.println(task.getStart()+" changed to "+start);
                task.setStart(start);

                System.out.println("Enter new Task End Date(yyyy-mm-dd) (" + task.getEnd() + ")");
                String end = input.nextLine();
                System.out.println(task.getEnd()+" changed to "+end);
                task.setEnd(end);

                System.out.println("Enter new Task Phase\n1-DONE\n2-DOING\n3-TODO (" + task.getPhase() + ")");
                String phase = input.nextLine();
                String oldPhase=task.getPhase().toString();
                task.setPhase(phase);
                System.out.println(oldPhase+" changed to "+task.getPhase());
